...
This repository is a ROS 2 workspace rooted at `/home/konst/ros2_ws`.

Canonical ROS 2 layout:
...
- `src/` contains the source packages discovered by `colcon`
- `build/` contains temporary build artifacts generated by `colcon build`
- `install/` contains the workspace overlay that you source before running ROS 2 commands
- `log/` contains build logs

Packages currently in this workspace:

- `my_robot_interfaces` in `src/my_robot_interfaces`
- `my_py_pkg` in `src/my_py_pkg`
- `my_cpp_pkg` in `src/my_cpp_pkg`
- `my_rust_pkg` in `src/my_rust_pkg`

`colcon` discovers packages from `src/`, so custom packages should live there.
...
- `source install/setup.bash` overlays your local workspace so ROS 2 can see packages built from this workspace

If you open a new terminal, run the setup commands again before using `ros2 run`,
`ros2 topic`, `ros2 node`, or `ros2 interface`.

## Custom interfaces

This workspace defines a custom message in `src/my_robot_interfaces/msg/HardwareStatus.msg`.

The interface package is generated in `src/my_robot_interfaces/CMakeLists.txt` with `rosidl_generate_interfaces(...)`.
